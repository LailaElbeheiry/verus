    ...
    pub(crate) fn finalize_exp(&self, _ctx: &Ctx, exp: &Exp) -> Result<Exp, VirErr> {
        let exp = map_exp_visitor(exp, &mut |exp| match exp.e() {
            ExpX::Var(x) if self.rename_delayed.contains_key(x) => {
                SpannedTyped::new_tagged(&exp.span, &exp.typ, ExpX::Var(self.rename_delayed[x].clone()))
            }
    ...
    match &expr.x {
        ...
        ExprX::Unary(op, exprr) => {
                ...
                (state.checking_recommends(ctx), op)
            {
                ...
                let has_type = SpannedTyped::new_tagged(&expr.span, &Arc::new(TypX::Bool), has_type);
                            ...
                            (Mode::Exec, true, TypX::Int(ir)) if ir.is_bounded() => {
                                let (assert_exp, msg) = match arith {
                                    ...
                                    ArithOp::EuclideanDiv | ArithOp::EuclideanMod => {
                                        ...
                                        let ne =
                                            ExpX::Binary(BinaryOp::Ne, e2.clone(), e2.new_x_tagged(zero));
            ...
            match exp {
                ReturnValue::Some(mut exp) if is_pure_exp => {
                    ...
                    for bnd in binds.iter().rev() {
                        ...
                        exp = SpannedTyped::new_tagged(&expr.span, &exp.typ, ExpX::Bind(bnd, exp.clone()));
                    }
                    assert!(!never_return);
