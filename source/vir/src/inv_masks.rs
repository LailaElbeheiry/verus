    ...
    pub fn to_exp(self: &Self, ctx: &Ctx) -> Exp {
        match self {
            MaskSet::Empty { span } => {
                ...
                let empty_exp = SpannedTyped::new_tagged(&span, &namespace_set_typ(ctx), empty_expx);
                empty_exp
            }
            MaskSet::Full { span } => {
                let full_fun =
                ...
                let full_exp = SpannedTyped::new_tagged(&span, &namespace_set_typ(ctx), full_expx);
                full_exp
            }
            MaskSet::Insert { base, elem } => {
                ...
                let insert_exp =
                    SpannedTyped::new_tagged(&elem.span, &namespace_set_typ(ctx), insert_expx);
                insert_exp
            }
            MaskSet::Remove { base, elem } => {
                ...
                let remove_exp =
                    SpannedTyped::new_tagged(&elem.span, &namespace_set_typ(ctx), remove_expx);
                remove_exp
            }
    ...
    fn contains_internal(
        ...
        match self {
            ...
            MaskSet::Remove { base, elem: removed } => {
                ...
                let neq_exp = SpannedTyped::new_tagged(&elem.span, &Arc::new(TypX::Bool), neq_expx);
            ...
            _ => {
                ...
                let contains_exp =
                    SpannedTyped::new_tagged(&elem.span, &Arc::new(TypX::Bool), contains_expx);
        ...
        match self {
            ...
            _ => match other {
                ...
                _ => {
                    ...
                    let subset_of_exp =
                        SpannedTyped::new_tagged(&call_span, &Arc::new(TypX::Bool), subset_of_expx);
