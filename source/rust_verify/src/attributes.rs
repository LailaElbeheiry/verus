...
pub(crate) enum Attr {
    ...
    AssumeTermination,
    // (automation) labels functions that define imaginary fields to help with `guard_effects`
    ImaginaryFields,
    // (automation) labels functions that define enum accessors to help with `guard_effects`
    EnumAccessor,
}
    ...
    for (prefix, span, attr) in attrs_to_trees(attrs)? {
        ...
        match prefix {
            AttrPrefix::Verifier => match &attr {
                ...
                AttrTree::Fun(_, name, None) if name == "enum_accessor" => {
                    v.push(Attr::EnumAccessor);
                }
...
pub(crate) fn get_verifier_attrs_maybe_check(
    ...
    for attr in parse_attrs(attrs, diagnostics)? {
        match attr {
            ...
            Attr::EnumAccessor => vs.enum_accessor = true,
            _ => {}
        }
    }
    if do_check {
